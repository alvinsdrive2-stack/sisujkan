  ...
  const loadData = async () => {
    ...
    try {
      const [jabkerRes, skemaRes] = await Promise.all([
        fetch(`${API_BASE_URL}/validator/jabker`, { headers: { Accept: "application/json", Authorization: `Bearer ${g("access_token")}` } }),
        fetch(`${API_BASE_URL}/validator/skema`, { headers: { Accept: "application/json", Authorization: `Bearer ${g("access_token")}` } }),
      ])
    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        ...
        <div className="bg-orange-50 dark:bg-orange-900/20 rounded-lg p-4 border border-orange-100 dark:border-orange-800">
          <p className="text-sm text-orange-600 dark:text-orange-400">Perlu Review</p>
          <p className="text-3xl font-bold text-orange-800 dark:text-orange-200">-</p>
        </div>
      </div>

      {loading ? (
        <div className="border border-dashed border-slate-300 rounded-lg p-12 text-center"><p className="text-slate-400">Memuat data...</p></div>
      ) : jabkerList.length === 0 ? (
